 ...
 */
class KeyboardChangeListener implements ViewTreeObserver.OnGlobalLayoutListener {
    ...
    private int bottomSoftHeight=0;
    ...
    public void onGlobalLayout() {
        ...
        if (keyboardHeight < MIN_KEYBOARD_HEIGHT) {
            bottomSoftHeight = keyboardHeight;
        }
        keyboardHeight=keyboardHeight-bottomSoftHeight;

        //activity visible frame
        /*Rect rectAct = new Rect();
        mContentActivity.getWindow().getDecorView().getWindowVisibleDisplayFrame(rectAct);
        WindowManager wm = (WindowManager) mContentActivity
                .getSystemService(Context.WINDOW_SERVICE);
        DisplayMetrics outMetrics = new DisplayMetrics();
        wm.getDefaultDisplay().getMetrics(outMetrics);
        System.out.println(outMetrics.heightPixels);
        int screenHeight=mContentActivity.getWindow().getDecorView().getRootView().getHeight();
        int activityHeight=outMetrics.heightPixels;
        int heightDifference = screenHeight - rectAct.bottom -126;
